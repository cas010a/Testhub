...
def export_xmind(request):
    ...
    请求体: {
        "title": "任务标题",
        "test_cases": [
            {"caseId": "TC001", "scenario": "...", "precondition": "...",
    ...
    for index, tc in enumerate(test_cases, 1):
        ...
        steps = clean_text(tc.get('steps'))
        expected = clean_text(tc.get('expected'))
        priority = clean_text(tc.get('priority')) or 'P2'
    ...
    filename = f'AI生成测试用例_{date_str}.xmind'

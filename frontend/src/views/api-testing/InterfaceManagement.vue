        ...
        <div v-else class="request-detail">
          ...
          <el-tabs v-model="activeTab" class="request-tabs">
            ...
            <template v-if="!selectedRequest || selectedRequest.request_type !== 'WEBSOCKET'">
              ...
              <el-tab-pane :label="$t('apiTesting.interface.extractRules')" name="extract">
                <div class="extract-editor">
                  <div class="extract-header">
                    <el-button size="small" type="primary" @click="addExtractRule">
                      <el-icon><Plus /></el-icon>
                      {{ $t('apiTesting.interface.addExtractRule') }}
                    </el-button>
                    <span class="extract-hint">{{ $t('apiTesting.interface.extractHint') }}</span>
                  </div>
                  ...
                  <div class="extract-list">
                    ...
                    >
                      <el-input
                        v-model="rule.name"
                        :placeholder="$t('apiTesting.interface.extractRuleName')"
                        size="small"
                        style="width: 150px; flex-shrink: 0"
                      />
                      <el-select v-model="rule.source" size="small" style="width: 100px; flex-shrink: 0">
                        <el-option :label="$t('apiTesting.interface.extractSourceBody')" value="body" />
                        <el-option :label="$t('apiTesting.interface.extractSourceHeader')" value="header" />
                      </el-select>
                      <el-input
                        v-model="rule.expression"
                        :placeholder="$t('apiTesting.interface.extractRuleExpression')"
                        size="small"
                        style="flex: 1"
                      />
                      <el-button link type="danger" :icon="Delete" @click="removeExtractRule(index)" />
                    </div>
                  </div>

                  <div v-if="!selectedRequest.extract_rules || selectedRequest.extract_rules.length === 0" class="no-extract">
                    <p>{{ $t('apiTesting.interface.noExtractRules') }}</p>
                  </div>
                </div>
              </el-tab-pane>
            </template>
          ...
          <div v-if="response" class="response-section">
            ...
            <div v-if="response.extracted_variables && Object.keys(response.extracted_variables).length > 0" class="extracted-vars">
              <div class="extracted-vars-title">
                <el-icon><MagicStick /></el-icon>
                {{ $t('apiTesting.interface.extractedVars') }}
              </div>
              <div v-for="(value, name) in response.extracted_variables" :key="name" class="extracted-var-item">
                <el-tag size="small" type="success">{{ name }}</el-tag>
                <code class="extracted-var-value">{{ value }}</code>
                <span class="extracted-var-tip">{{ $t('apiTesting.interface.extractedVarsTip', { name: '\u007b\u007b' + name + '\u007d\u007d' }) }}</span>
              </div>
            </div>
...
const sendRequest = async () => {
  ...
  try {
    ...
    const requestData = {
      ...
      extract_rules: selectedRequest.value.extract_rules || []
    }
...
const removeExtractRule = (index) => {
  if (!selectedRequest.value || !selectedRequest.value.extract_rules) return
